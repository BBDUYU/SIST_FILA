	...
	<input type="hidden" name="searchsCateNo" value />
		...
		<div id="contents" class="mbr__contents">
			...
			<section class="mbr-box">				
				<!-- 입력 폼 - 이메일, 비밀번호 -->
				<form id="loginForm" name="loginForm" action="verify_result.asp" method="post" target="dataFrame" class="on">
				<input type="hidden" name="rtnUrl" value="/main/main.asp">
				<input type="hidden" name="lgc" id="lgc" value="0">
				<div class="inp-box">
					<div>
						<input type="text" id="memberId1"  name="mb_id"  class="input id login_id" placeholder="아이디" title="아이디" value="" onClick="this.style.backgroundImage='none';" onkeyup="this.value=this.value.replace(/[^a-zA-Z-_0-9\s]/g,'');fnChkByte(this);" onFocus="this.style.backgroundImage='none'" maxlength="20" limitbyte="20" tabindex="51"/>
					</div>

					<div>
						<input type="password" id="memberPwd" class="inp__pw login_pw" name="password" placeholder="비밀번호" value="" onClick="this.style.backgroundImage='none'" onFocus="this.style.backgroundImage='none'" onkeydown="if (event.keyCode==13) loginCheck();void(0);" tabindex="52" />
						<button type="button" class="pwonoff__btn">on/off</button> <!-- 눈 감기 클래스 off -->
					</div>					
				...
				<!-- 로그인 버튼 -->
				<div class="btn-box">
					<a href="javascript:loginCheck();void(0);" class="btn_bg__bk" id="loginbtn" tabindex="56">로그인</a>
					<!--
					<a id="loginbtn2" style="display:none;" class="btn_bg__bk"><img src="/pc/resource/images/waiting.gif"></a>
					-->

					<!-- 2023-02-08 로딩 수정 -->
					<a id="loginbtn2" style="display:none;" class="btn_bg__bk _style_loading">
				...
				<div class="login_sns">
					...
					<!-- <a href="" class="btn_toss">토스로 로그인하기</a> -->
				</div>
				<!-- //sns 로그인 -->
...
//'''2024-09-26 프렌즈회원관련
var loginFriendsPop
function loginFriendsPop(url){
	jQuery.ajax({
		type: "POST",
		url: "/member/verify_friends.asp",
		dataType : "JSON",
		success: function(data) {
			var formStr = "";
				formStr +='	<div class="main_popup member-login__popup">';
				...
				formStr +='			</div>';
				formStr +='		<div class="close_area close_style2">';
				formStr +='			<a href=javascript:friendsGo("'+ url +'"); class="btn_close fl">프렌즈 로그인</a>';
				formStr +='			<a href="'+ url +'" class="btn_close">일반 로그인</a>';
				formStr +='		</div>'; 
				formStr +='	</div>';
