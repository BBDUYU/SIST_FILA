...
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.1, minimum-scale=1.0, user-scalable=no, target-densitydpi=medium-dpi">
	...
	<meta property="og:url" content="https://www.fila.co.kr/member/searchIDPW.asp" />
	...
	<link rel="canonical" href="https://www.fila.co.kr/member/searchIDPW.asp">
...
jQuery(function(){
	jQuery("input[onkeypress='validate2(event)']").removeAttr("onkeypress").attr("type","tel").keydown(function(e){
...
function keycheck(){
	console.log(document.idTelForm.mte2_1.value.length);
	if (document.idTelForm.membername.value != "" && document.idTelForm.mte2_1.value.length >= 7){
		$(".confirmCls").removeClass("btn_bg__gr");
		$(".confirmCls").addClass("btn_bg__bk");
	}
}
//-->
</SCRIPT>
					...
					<div class="inner">
						...
						<div class="keywords-box _popular">
							<div>
								...
								<p class="update-txt">11:00 업데이트</p>
							</div>
	...
	<input type="hidden" name="searchsCateNo" value />
		...
		<div id="contents" class="mbr__contents">
			...
			<section class="mbr-box">		
				<div class="join-sec">
					<p class="txt">카카오 연동을 하시면, 아이디 / 비밀번호 찾기 없이 바로 로그인이 가능합니다.</p>
					<div class="btn-box">
						<a href="javascript:snsLogin('KK');void(0);" class="join_kakao">카카오 연동 & 로그인</a>
					</div>
				</div>
				<div class="info-sec">
				<form name="idTelForm" id="idTelForm"method="post"action="<%=request.getContextPath()%>/member/searchId.do">

				
					<div class="inp-box">
						<!-- 이름 -->
						<div class="name">
							<input type="text" placeholder="이름" class="" name="membername" id="membername" data-val="이름"  maxlength="40" onkeypress='keycheck();'>
						</div>
						<!-- 휴대폰 -->
						<div class="phone">
							<!--select name="" id="" class="sel__style1 wid__style2">
								<option value="SKT">SKT</option>
								<option value="KT">KT</option>
								<option value="LG U+">LG U+</option>
							</select-->
							<select name="mte1_1" id="mte1_1" class="sel__style1 wid__style3">
							...
							<input type="number" maxlength="8" class="inp__phone" name="mte2_1" id="mte2_1" onkeypress='validate(event);keycheck();' >
						</div>
						<div class="self-number" id="confirm" style="display:none;"><!-- 인증번호 요청시 display: block -->
							<input type="text" placeholder="" class="" id="confirmNo" name="confirmNo" title="인증번호" value="" onClick="this.style.backgroundImage='none'" onFocus="this.style.backgroundImage='none'" onkeydown="if (event.keyCode==13) confirmCheck2();void(0);" MAXLENGTH="6" onkeypress="validate2(event)"> 
							<div class="inp_in">
								<small id="countdown" class="count count_down"></small>
								<button type="button" onclick="javascript:serachID2('tel');void(0);">재전송</button>
							</div>
						</div>
					</div>
				</form>
				...
				<div class="btn-box">
					<button type="submit"class="btn_bg__gr confirmCls"id="loginbtn"> 인증번호 요청</button>

					<a id="loginbtn2" style="display:none;" class="btn_bg__gr "><img src="/pc/resource/images/waiting.gif"></a>
					<a href="javascript:confirmCheck2();void(0);" id="confirmA" class="btn_bg__bk " style="display: none">인증번호 확인</a> <!-- 인증번호 확인시 display:block -->
				</div>
				<div class="btn_chk" style="display:none;"></div>
			</section>
			<!-- //아이디 / 비밀번호 찾기 -->
		</div>
		<!-- // end of :: contents -->
